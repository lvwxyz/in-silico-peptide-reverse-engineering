...
# (시간은 1초당 0.001배 감점, 메모리는 1MB당 0.5배 감점, 편차는 0.1배 감점)
W_STD = 0.01
W_TIME = 0.001
W_MEM = 0.5
...
def run_pipeline_B(model, aa_to_idx, embed_layer, config):
    ...
    # 초기 개체군 생성
    pop = [("".join(random.choices("ACDEFGHIKLMNPQRSTVWY", k=max(0, random.randint(0, 15-len(ref))))) + 
            ref + 
            "".join(random.choices("ACDEFGHIKLMNPQRSTVWY", k=max(0, random.randint(0, 15-len(ref))))))[:15] 
    ...
    for gen in range(generations):
        ...
        for seq in pop:
            ...
            cost = (w_ai * p_fbr) + (w_gr * abs(gravy)) + (w_ch * abs(charge)) - (w_cd * h_score)
...
def execute_sensitivity_test(model, aa_to_idx, embed_layer, base_config):
    ...
    for s_name, weights in scenarios.items():
        ...
        # 🔥 [수정 포인트] run_pipeline_B의 반환값(results, history)을 모두 받습니다.
        results, history = run_pipeline_B(model, aa_to_idx, embed_layer, temp_config)
        
        # [수정 포인트] results는 (final_metrics, best_seq) 구조입니다.
        final_metrics, best_seq = results
        
        # [수정 포인트] final_metrics는 (cost, p_fbr, gravy, charge, h_raw) 5개 요소입니다.
        cost, p_fbr, gravy, charge, motif = final_metrics
